...
describe('Dashboard API session contract', () => {
  ...
  test('does not expose retired system update services', () => {
    expect('getSystemUpdate' in api).toBe(false)
    expect('checkSystemUpdate' in api).toBe(false)
    expect('installSystemUpdate' in api).toBe(false)
    expect('cancelSystemUpdate' in api).toBe(false)
  })
})
