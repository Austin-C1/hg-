...
export function createBettingProcessController({
  cwd = process.cwd(), env = process.env, dbPath = env.CROWN_DB_PATH || DEFAULT_DB_PATH,
  ...
  async function startOperation(token, { restart = false, dbPath: actionDbPath = dbPath } = {}) {
    ...
    const expectedKeys = bettingRoleLeaseKeys({ cwd, dbPath: actionDbPath })
    ...
    const args = [path.resolve(cwd, 'scripts/crown-betting-worker.mjs'), '--db-path', actionDbPath || DEFAULT_DB_PATH,
      '--mode', 'real', '--worker-lease-key', requestedLeaseKey, '--generation', String(token), '--ready-nonce', nonce]
    const next = spawnCommand(process.execPath, args, {
      cwd, env, stdio: ['ignore', 'ignore', 'ignore', 'ipc'], windowsHide: true,
    })
    child = next
