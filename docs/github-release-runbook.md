...
当前生产 Preview/Submit/Reconciliation capability 必须保持 `8/4/0`：八个全场 main 方向可 Preview，赛前全场 main 的让球 home/away、大小球 over/under 可 Submit；滚球 Submit 与 Reconciliation 保持关闭。发布流程不得通过环境变量、配置或替换文件改变该矩阵，真实 runtime 默认 off。
...
- Watcher/worker 默认停止；capability 为 `8/4/0`，滚球 Submit 与 Reconciliation 关闭；
...
| 首次运行 | 创建空数据库；Watcher/worker off；capability `8/4/0`，滚球 Submit 与 Reconciliation 关闭 |
