...
- app/frontend/schema contract 为 `dynamic-betting-cards-v1`。canonical Crown preview/submit/reconciliation capability 仍为 `0/0/0`；旧 adapter/CLI 已删除，不能作为替代执行入口。
...
- 旧 adapter/CLI 已删除；生产 `CrownAccountExecutionProvider` 在 canonical submit capability 为 0 时零网络拒绝。
...
- worker 的 `off|preview|simulated` 只用于离线调试；real worker 只能由 Dashboard 父进程通过私有 ready-ticket/GO IPC 启动。canonical Crown preview/submit/reconciliation 能力仍为 0，补齐 exact 证据与当次授权前不会运行。
...
# C4 readiness gate（2026-07-12）

- Canonical capability matrix：`crown-protocol-capabilities-v2:23628f891d1edb9a`。
- Exact coverage：preview `0`、submit `0`、reconciliation `0`。
...
- 当前 live readiness：`blocked`。没有任何 capability row 同时具备 verified preview+submit+reconciliation，因此不得启用真实 Crown 网络提交。

## 2026-07-13 Task 6 canonical evidence preparation
...
- 离线 focused `86/86`、相关 parser/mapper/evidence `96/96`、backend `1092/1092`、security `5/5`、syntax `217` 通过；最终独立复审 Critical/Important/Minor `0/0/0`。Canonical matrix 仍为 preview/submit/reconciliation `0/0/0`，Task 7 未获当次授权前不得访问真实 Preview。
