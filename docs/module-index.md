...
- `crown-capability-matrix.mjs`、strict mapper/parser、account providers 与 B2 attempt 是强绑定执行边界；canonical Preview/Submit/Reconciliation 为 `8/4/0`，赛前全场 main 的让球 home/away、大小球 over/under 允许 Submit。
...
- `20260715-prematch-submit-accepted.safe.json` 只保存脱敏 accepted promotion 证据；Reconciliation 仍因缺少结果查询证据关闭。
- 多账号 Preview 使用 `Promise.allSettled` 并发收集容量；Submit 保持逐 child 的持久 lock、fence 与 at-most-once。已验证账户 policy 可提供 CNY integer stake step。
...
- 当时为 capture `20260714-085221` 生成 safe candidate：4 条记录、6 个排除字段、8 个 HMAC binding；因 `rejected-attempt-required`、`integer-cny-step-unproven`、`exact-capability-unproven` 保持 evidence incomplete，capability 当时为 `0/0/0`。当前值以顶部 Task 3 的 `8/4/0` 为准。
...
- 该历史阶段的生产 Preview/Submit capability 为 `0/0`。随后在 2026-07-14 完成一笔受控 accepted `FT_bet`；当前 capability 以顶部 Task 3 的 side-aware `8/4/0` 为准。
...
- 历史阶段 Contract/capability：app/frontend/schema 均为 `dynamic-betting-cards-v1`；canonical Crown Preview/Submit/Reconciliation 当时为 `0/0/0`，当前值以顶部 Task 3 的 `8/4/0` 为准。

## 历史：2026-07-12 C 阶段完成与运行数据策略

- C Task 1–11 当时已完成，历史代码验证基线为 backend 874/874、syntax 180、frontend 66/66；真实投注当时被 capability `0/0/0` 阻断，当前值以顶部 Task 3 的 `8/4/0` 为准。
...
- 当时生产 Crown capability matrix 的 preview/submit 均为 0，真实提交和自动对账 fail-closed；offline fixture 不能证明 Crown batch。该历史阶段未执行真实 preview、`FT_bet` 或 Telegram 发送；当前 capability 以顶部 Task 3 的 side-aware `8/4/0` 为准。
...
- B2 Task 11–12 当时已完成 sanitized fixture/offline-only 验收；该历史阶段 canonical Crown preview/submit 能力为 0。当前 capability 以顶部 Task 3 的 side-aware `8/4/0` 为准。
