...
- 手动“检测账号”可使用该账号已保存且通过验证的 public HTTPS exact origin；这一放宽只作用于 fresh login + `get_game_list` + `get_member_data`，生产 preview/submit Provider 仍要求环境 allowlist/capability。
...
- 登录检测只允许账号配置的 public HTTPS exact origin，且必须列入 `CROWN_BETTING_ALLOWED_ORIGINS`；API 不返回密码、cookie、session、ticket 或原始响应。
